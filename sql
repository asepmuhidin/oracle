select orders.order_id,cust.cust_first_name,cust.cust_street_address1,
...
select prod.product_id,product_name,quantity,unit_price,
quantity*unit_price as subTotal
from demo_order_items doi inner join demo_product_info prod
on doi.product_id=prod.product_id

select orders.customer_id, cust.cust_first_name,cust.cust_street_address1,
sum(order_detail.quantity*order_detail.Unit_Price) as grand_total
